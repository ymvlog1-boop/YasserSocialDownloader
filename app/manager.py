...
from .filenames import clean_gallery_suffix, clean_video_suffix
...
class DownloadManager(QObject):
    ...
    def parse_line(self, task, line):
        ...
        if line.startswith('YPROGRESS:'):
            ...
            path = line[6:]
            if path:
                if task.get('engine') == 'gallery':
                    path = clean_gallery_suffix(path)
                elif task.get('engine') in ('video', 'facebook'):
                    path = clean_video_suffix(path)
