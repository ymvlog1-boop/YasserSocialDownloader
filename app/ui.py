...
class Window(QMainWindow):
    def __init__(self,store):
        ...
        self.startup_update_started=False
        QTimer.singleShot(2500,self.check_startup_update)
    ...
    def queue_page(self):
        ...
        self.detail=label('اختر مهمة لعرض حالتها','muted'); v.addWidget(self.detail); self.queue.itemSelectionChanged.connect(lambda:self.remember_selection(self.queue)); self.queue.itemSelectionChanged.connect(self.show_detail)
        r=QHBoxLayout()
    ...
    def remember_selection(self,table):
        row=table.currentRow()
        if row>=0 and table.item(row,0) is not None:
            self.selected_task_ids[id(table)]=table.item(row,0).data(Qt.UserRole)
    def selected(self,table):
        row=table.currentRow(); task_id=None
        if row>=0 and table.item(row,0) is not None:
            task_id=table.item(row,0).data(Qt.UserRole)
            self.selected_task_ids[id(table)]=task_id
        else:
            task_id=self.selected_task_ids.get(id(table))
        task=next((t for t in self.manager.tasks if t['id']==task_id),None)
        if task is None:self.selected_task_ids.pop(id(table),None)
        return task
    ...
    def refresh(self):
        for table,tasks in [(self.queue,self.manager.tasks),(self.history,[t for t in self.manager.tasks if t['state'] in ('completed','partial','failed','cancelled','skipped','empty')])]:
            ...
            for row,t in enumerate(tasks):
                ...
                if id==t['id']:
                    table.setCurrentCell(row,0);table.selectRow(row)
    ...
    def check_startup_update(self):
        if self.startup_update_started:return
        self.startup_update_started=True
        from .updater import AppUpdateCheckWorker
        self.auto_update_checker=AppUpdateCheckWorker();self.auto_update_checker.result.connect(self.startup_update_result);self.auto_update_checker.start()
    def startup_update_result(self,action,value):
        if action!='available':return
        answer=QMessageBox.question(
            self,'تحديث جديد متوفر',
            'يتوفر الإصدار '+value+' من Yasser Social Downloader.\n\nهل تريد تنزيله وتثبيته الآن؟\nلن يتم تغيير أي ملف بدون موافقتك.',
            QMessageBox.Yes|QMessageBox.No,QMessageBox.Yes)
        if answer==QMessageBox.Yes:
            self.go(4);self.update_engines()
    ...
    def closeEvent(self,event):
        ...
        if hasattr(self,'auto_update_checker') and self.auto_update_checker.isRunning():
            self.auto_update_checker.wait(9000)
