...
def _gallery_args(task, store, cookie_path, target_url):
    ...
    # Conservative filenames: only use metadata keys known for these extractors.
    if platform == 'instagram':
        args += ['--filename', 'instagram_{sidecar_media_id:?/_/}{media_id}.{extension}']
        if profile:
        ...
        args += ['--filename', 'twitter_{tweet_id}_{num}.{extension}']
        if profile:
    ...
    if media == 'images':
        args += ['--filter', f'extension not in {video_exts}']
        if platform == 'tiktok':
        ...
        args += ['--filter', f'extension in {video_exts}']
        if platform == 'instagram':
...
def _video_args(task, store, cookie_path, target_url):
    ...
    if task.get('platform') == 'tiktok' and task.get('phase') == 'tiktok_ytdlp':
        ...
        if task['quality'] == 'best':
            fmt = 'bv*[acodec=none]+ba/b[acodec!=none]'
        elif task['quality'] == 'audio':
            fmt = 'ba/b[acodec!=none]'
        else:
            q = task['quality']
            fmt = f'bv*[height<={q}][acodec=none]+ba/b[height<={q}][acodec!=none]'
    else:
    ...
    args = [
        ...
        '-o', '%(extractor)s_%(id)s_%(title).80s.%(ext)s', '-f', fmt,
...
def command(task, store, cookie_path=None):
    ...
    if task['platform'] == 'direct':
        if task['cookies']:
            raise ValueError('الروابط المباشرة لا تستخدم ملفات الكوكيز')
        prefix = [sys.executable, '--engine', 'direct'] if getattr(sys, 'frozen', False) else [sys.executable, str(resource() / 'main.py'), '--engine', 'direct']
        return prefix + ['--url', task['url'], '--folder', task['folder'], '--force' if task['force'] else '--resume'], 'direct'
