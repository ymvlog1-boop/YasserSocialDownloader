...
PLATFORMS = {'instagram': 'إنستغرام', 'twitter': 'إكس / تويتر', 'facebook': 'فيسبوك', 'tiktok': 'تيك توك', 'direct': 'رابط وسائط مباشر'}
...
def detect(url, direct=False):
    ...
    if direct:
        if p.scheme != 'https' or not host or host == 'localhost' or host.endswith('.localhost'):
            raise ValueError('الرابط المباشر يتطلب HTTPS واسم موقع عام')
...
def new_task(url,folder,media='auto',quality='best',cookies=False,force=False,direct=False):
    platform,url=detect(url,direct=direct)
    if direct and cookies:
