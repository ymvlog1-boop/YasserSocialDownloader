...
class Window(QMainWindow):
    ...
    def home(self):
        v=self.page('كل وسائطك، في مكان واحد','إنستغرام • إكس / تويتر • فيسبوك • تيك توك • روابط صور وفيديو مباشرة')
        ...
        self.direct=QCheckBox('روابط مباشرة لملفات صور أو فيديو (HTTPS)'); self.direct.toggled.connect(self.detect_input); v.addWidget(self.direct)
    ...
    def option_state(self):
        self.media.setEnabled(not self.direct.isChecked())
        self.quality.setEnabled(self.media.currentData()=='video' and not self.direct.isChecked())
        self.use_cookie.setEnabled(not self.direct.isChecked())
        self.notice.setText('الوضع المباشر يقبل ملف صورة أو فيديو واحدًا لكل رابط HTTPS، دون صفحات أو حسابات. احتفظ بحق تنزيل المحتوى.' if self.direct.isChecked() else 'الاستكمال يعتمد على دعم الموقع والمحرك. ملفات الحسابات قد تتطلب كوكيز.')
    def detect_input(self):
        try:
            platforms={detect(x,direct=self.direct.isChecked())[0] for x in self.urls.toPlainText().splitlines() if x.strip()}; self.platform_label.setText('نوع الرابط: '+ ' • '.join(PLATFORMS[p] for p in platforms) if platforms else 'جاهز لاستقبال الروابط')
    ...
    def enqueue(self,start):
        ...
        try:
            for url in lines:
                p,_=detect(url,direct=self.direct.isChecked())
                if self.use_cookie.isChecked() and p!='direct':
                ...
                t=new_task(url,folder,self.media.currentData(),self.quality.currentData() if self.media.currentData()=='video' else 'best',self.use_cookie.isChecked() and p!='direct',self.force.isChecked(),direct=p=='direct')
    ...
    def action(self,table,a):
        ...
        if a=='pause':self.manager.stop(t)
            ...
            if t['platform']=='direct':return self.info('الروابط المباشرة لا تستخدم الكوكيز.')
    ...
    def settings_page(self):
        ...
        org=QCheckBox('تنظيم الملفات في مجلد مستقل لكل منصة');org.setChecked(self.store.get('organize',True));org.toggled.connect(lambda x:self.store.set('organize',x));v.addWidget(org)
    ...
    def about_page(self):
        ...
        v.addWidget(label('الإيقاف يوقف عملية التنزيل؛ الاستكمال يعيد تشغيل المحرك باستخدام الأجزاء والأرشيف حيث يتوفر الدعم. لا تُحذف الأجزاء عند الإلغاء.','muted'));v.addWidget(label('استعمل البرنامج لحفظ المحتوى الذي تملك حق الوصول إليه وتنزيله. لا يطلب البرنامج اسم مستخدم أو كلمة مرور ولا يجمع قياسات استخدام.','muted'))
